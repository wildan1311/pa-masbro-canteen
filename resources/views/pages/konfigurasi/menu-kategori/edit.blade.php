        ...
        <div class="content-wrapper">
            <div class="card">
                <div class="card-header">
                    <h4>Edit Menu Kategori {{ $menus->nama }}</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('menu-kategori.update', $menus->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col">
                                <div class="mb-3">
                                    <label for="basicInput" class="form-label">Nama Menu Kategori</label>
                                    <input type="text" placeholder="Input Here" class="form-control" id="basicInput"
                                        name="nama" value="{{ $menus->nama }}">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="kategori_id" id="" class="form-control">
                                        @foreach ($kategori as $kategor)
                                            <option value="{{ $kategor->id }}" {{$menus->kategori->id == $kategor->id ? 'selected' : ''}}>{{ $kategor->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
